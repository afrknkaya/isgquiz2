...
def parse_correct_answers_from_txt(file_path):
    """
    Belirtilen TXT dosya yolundan soru numarasını ve doğru cevap eşleştirmesini ayrıştırır.
    Format: '500: A'
    """
    if not os.path.exists(file_path):
...
def handle_option_change(questions, correct_answers):
    ...
    # Callback içinde gereksiz st.rerun() kaldırıldı
    # Streamlit, on_change sonrası zaten otomatik olarak yeniden çalışır.
    ...
    if questions: 
        ...
        if not st.session_state.exam_mode_active and not st.session_state.review_exam_incorrect_active:
            if st.session_state.feedback_trigger == 'correct':
                ...
                time.sleep(0.5) # Gecikme süresi 0.5 saniyeye düşürüldü
